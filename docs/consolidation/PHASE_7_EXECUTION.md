...
**Status: Batches 7.1–7.5 — implemented on the feature branch (7.1's + 7.3's usability passes and 7.5's manual assistive-technology matrix remain owner/tester-run; the automated a11y checks are enforced in the suite).** 7.1 unified the lexicon, the four-axis progress model, 2D/3D reader parity, and the one-time revisitation hint. 7.2 made the reader **hash-addressable** (browser Back closes it; passages bookmarkable) while preserving `useDialogFocus` and exact visit semantics (a `restoreStoryView` path that never records a spurious visit), added reliable scroll restoration for interrupted reads, a line-height preference, back-to-top, and a print stylesheet — keeping passages whole per the measured evidence. 7.3 added an include/exclude-adaptation-notes export preference + a milestone "save your journey" export at endings, keeping "Why this version?" a collapsed secondary disclosure. 7.4 gave the reader **ownership of their journey** — a guarded new-journey/reset, a machine-readable save import/export (migration-aware, distinct from the literary Markdown export), and honest non-blocking recovery for corrupt saves (quarantine), storage-quota failures, and save migrations. 7.5 enforced the inclusive-design bar: automated axe/keyboard/forced-colours e2e, a **linear passage-list alternative on the 2D map** (shared with the 3D list) so no critical task needs the spatial graph or WebGL, and a public accessibility statement + WCAG 2.1 AA release checklist with a manual AT protocol the owner runs. No contract identity has moved (the new preferences are additive/defaulted; the save schema stays `1.3.0`; `@axe-core/playwright` is a dev-only dependency).
...
## Batch 7.4 — Persistence, recovery, and reader control ([#174](https://github.com/zekusmaximus/Narramorph/issues/174))

**Complete on the feature branch.** The persistence machinery was solid but only half-connected: `clearProgress`/`exportProgress`/`importProgress` existed on the store yet reached no reader-facing UI, `importProgress` bypassed the migration engine, and quota + corrupt-save failures were swallowed silently. 7.4 connected the surface and made the failure paths honest. Design + owner-confirmed decisions: [PHASE_7_4_PERSISTENCE.md](PHASE_7_4_PERSISTENCE.md). Interface chrome only (ADR 0002); nothing uploaded; **save schema kept at `1.3.0`** (no required persisted field added).

**What shipped:**

- **Guarded "new journey" / reset.** A `JourneyControlActions` section in the progress dialog with a confirmation guard and an export-first offer; confirming calls the existing `clearProgress`, which resets reading progress but **keeps reading preferences** (theme, text size, line height, reduce-motion, export-notes) — owner Fork B.
- **Machine-readable save import/export (distinct from the literary Markdown export).** `useJourneySaveFile` downloads the JSON save via the existing `downloadTextFile`; import reads a file and routes it through the store's now **migration-aware** `importProgress` (`prepareSavedState`) — fixing the pre-7.4 bypass so an older-schema file migrates up. Import replaces the in-progress journey only behind a confirm + export-first guard (owner Fork D) and returns a typed `ImportProgressResult` so the UI reports success (with any migrations) or failure (`parse`/`invalid`) without touching the current journey on failure.
- **Corrupt-save recovery (quarantine, non-blocking).** The repository now reads the raw stored string, so it distinguishes an empty slot from a corrupt one (non-JSON _or_ structurally invalid). On `invalid`, `loadProgress` **quarantines** the raw bytes to a device-local `narramorph-saved-state.corrupt` key and clears the primary slot (so the app starts clean instead of re-failing every load), and surfaces a dismissible notice offering the raw blob for download — owner Fork C.
- **Honest storage-quota + migration signals.** `saveProgress` threads the repository's success boolean into a `lastSaveFailed` flag (a swallowed `QuotaExceededError` becomes a visible, dismissible reader notice with a save-file escape); a migrated load records `lastLoadMigrations` and shows a brief local "we updated your save" notice. Both render in a non-blocking `PersistenceNotices` status region over the map — never a modal. All local; nothing uploaded.

### Gate evidence (local, Node 22, on the feature branch)

- `type-check`: pass. `lint:ci`: **0 errors / 0 warnings** (baseline held).
- `test:run`: **443 tests pass** (+25 over 7.3: repository raw-boundary + quarantine, store `persistence.test` for migration-aware import / corrupt recovery / quota / preference-preserving reset, and `JourneyControlActions` + `PersistenceNotices` component tests).
- `story:package:validate`: `eternal-return@1.3.0` `80f3d5a2…` (unchanged). `canon:strict`: **errors=0**, warnings=6116, waived=31. `literary`/`slice`: valid against `v1.0.2`. `content:validate:runtime`: 8.
- `build`: pass. `bundle:check`: pass — CSS **70.51 KiB / 13.33 KiB gzip**, under the 72,500 / 13,700 budget (the new controls/notices were consolidated to reuse existing utility classes rather than bump the budget). **No package or save-schema identity moved.**
- Playwright via the throwaway sandbox-Chromium (1194) override config (deleted, never committed): a new `reader-persistence` e2e drives the full acceptance round-trip (read a passage → export a save → guarded new-journey clears progress → import the save → progress restored); the `accessibility-confidence` keyboard walk was extended over the new controls and stays trap-contained. Functional suite passed, **real exit code 0**.

## Batch 7.5 — Manual accessibility & inclusive-design validation ([#175](https://github.com/zekusmaximus/Narramorph/issues/175))

**Complete on the feature branch (the manual AT matrix is owner/tester-run — its results table is present and filled only when run).** The base was already strong — a fully keyboard-driven 2D reader with contained focus traps, app-wide reduced motion, a non-WebGL default map, and a plain-DOM passage list for the 3D view. 7.5 raised and enforced the floor. Design + owner-confirmed decisions: [PHASE_7_5_ACCESSIBILITY.md](PHASE_7_5_ACCESSIBILITY.md). Meta-docs / interface chrome only (ADR 0002); **no package or save-schema identity change**. Acceptance gate: _no critical task requires sight, pointer precision, colour alone, motion, or WebGL._

**What shipped:**

- **Automated WCAG scanning (owner Fork A).** Added `@axe-core/playwright` (dev-only) and `e2e/accessibility-audit.spec.ts`: axe scans of the landing/map, the open reader, the progress + settings dialogs, and the expanded passage list — failing on any serious/critical WCAG 2.1 A/AA violation — plus a forced-colours operability check. (The 3D WebGL view is scanned in real browsers, not the GPU-less CI sandbox; its list is the same shared component, scanned in 2D.)
- **A linear passage-list alternative on the 2D map (owner Fork B).** Extracted a shared `PassageListNav` (bound to the 2D or 3D interaction adapter) from the 3D `SceneNodeList`; the 2D map now carries a **collapsible** "Passage list" — a non-spatial, keyboard/screen-reader path that opens the same reader a graph click would. `e2e/accessibility-semantic-journey.spec.ts` drives a full list-navigable, WebGL-free, keyboard-only journey (open a passage from the list → follow a branch).
- **Public accessibility statement + release checklist (owner Fork C).** `docs/ACCESSIBILITY.md` — WCAG 2.1 AA target, design-for-access summary, what is tested (automated + the manual matrix), plainly-stated known limitations (3D is an enhancement with a DOM alternative; the map is a complex graph with a linear list), a feedback channel (the repo issue tracker), and an evidence-linked WCAG 2.1 AA checklist. A discreet **footer link** points to it.
- **Manual AT protocol (owner Fork D — owner/tester-run, never fabricated).** The full matrix (NVDA + Firefox/Chrome on Windows; VoiceOver + Safari on macOS + iOS; TalkBack + Chrome on Android; plus forced-colours, 200% zoom, text-spacing) with task scripts and an **empty results table** to fill when run.

### Gate evidence (local, Node 22, on the feature branch)

- `type-check`: pass. `lint:ci`: **0 errors / 0 warnings** (baseline held).
- `test:run`: **443 tests pass** (the 3D `SceneNodeList` refactor to the shared `PassageListNav` keeps its existing tests green; the new 2D list is covered end-to-end).
- `story:package:validate`: `eternal-return@1.3.0` `80f3d5a2…` (unchanged). `canon:strict`: **errors=0**, warnings=6116, waived=31. `literary`/`slice`: valid against `v1.0.2`. `content:validate:runtime`: 8.
- `build`: pass. `bundle:check`: pass — CSS **70.71 KiB / 13.37 KiB gzip**, under the 72,500 / 13,700 budget (the 2D list reuses the shared list's existing utility classes rather than bumping the budget). **No package or save-schema identity moved**; `@axe-core/playwright` is a dev-only dependency (not shipped).
- Playwright via the throwaway sandbox-Chromium (1194) override config (deleted, never committed): the new `accessibility-audit` (axe + forced-colours) and `accessibility-semantic-journey` specs pass, and the `accessibility-confidence` keyboard walk still holds with the new 2D list present. Functional suite green, **real exit code 0**. (axe found **no** serious/critical violations on the covered 2D surfaces.)
