...
**Status: Batches 7.1, 7.2, 7.3, and 7.4 — implemented on the feature branch (7.1's + 7.3's usability passes and the 7.5 manual-AT passes remain owner/tester-run).** 7.1 unified the lexicon, the four-axis progress model, 2D/3D reader parity, and the one-time revisitation hint. 7.2 made the reader **hash-addressable** (browser Back closes it; passages bookmarkable) while preserving `useDialogFocus` and exact visit semantics (a `restoreStoryView` path that never records a spurious visit), added reliable scroll restoration for interrupted reads, a line-height preference, back-to-top, and a print stylesheet — keeping passages whole per the measured evidence. 7.3 added an include/exclude-adaptation-notes export preference + a milestone "save your journey" export at endings, keeping "Why this version?" a collapsed secondary disclosure. 7.4 gave the reader **ownership of their journey** — a guarded new-journey/reset, a machine-readable save import/export (migration-aware, distinct from the literary Markdown export), and honest non-blocking recovery for corrupt saves (quarantine), storage-quota failures, and save migrations. No contract identity has moved (the new preferences are additive/defaulted; the save schema stays `1.3.0` — 7.4 added no required persisted field).
