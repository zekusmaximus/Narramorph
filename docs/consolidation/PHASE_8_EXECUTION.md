...
**Design proposed on the feature branch; awaiting owner decisions.** Design: **[PHASE_8_4_RELEASE_ROLLBACK.md](PHASE_8_4_RELEASE_ROLLBACK.md)**. Grounded audit: app version `0.1.0` lives in **three lockstep sites** (`package.json`, `saveState.ts`, `story-package.ts`); no release-manifest/ checksum/app-rollback tooling exists (`content:rollback` is package-only); no `_redirects`/`404.html`; assets are already Vite-hashed/immutable; **rollback is save-safe by design** (save schema frozen `1.3.0`; `prepareSavedState` normalizes `appVersion`); Cloudflare Pages retains deployments → instant edge rollback. Plan: a release-manifest generator (app version, package `80f3d5a2…`, concordance `c779795f…`, literary `v1.0.2`, save `1.3.0`, build SHA, asset checksums, app↔package compatibility) + a lockstep check; immutable-asset/HTML caching + `_redirects` + a branded `404.html`; release-notes + `SHA256SUMS`; a one-command rollback script + runbook + a save-safety forward/back-compat test; and a `verify:reproducible` double-build check. Owner forks: (1) **app version** bump `0.1.0`→`0.2.0` (recommend) vs. keep; (2) canonical host apex/www (DNS); (3) HTML `no-cache` + immutable assets (recommend); (4) rollback = Cloudflare instant promote (recommend). Owner-gated: host/DNS provisioning, preview/staging/production, and the rollback rehearsal. **Package/concordance/literary/save identities stay frozen — only the app version may move.**
