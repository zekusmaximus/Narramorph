...
- **Caching / redirects / 404** — `public/_headers` gains immutable `/assets/*` + `/fonts/*` and `no-cache` HTML; `public/_redirects` (`www.narramorph.com → narramorph.com` 301); a branded `public/404.html`. `.gitignore` updated so `_redirects`/`404.html` are tracked (Vite copies them to the deploy root).
...
**Design proposed on the feature branch; awaiting owner confirmation.** Design: **[PHASE_8_5_PERFORMANCE.md](PHASE_8_5_PERFORMANCE.md)**. Grounded audit: most of 8.5's scope is **already covered** — budgets/lazy-loading (`performance-boundaries`), every ending (`phase-3-path-coverage`), WebGL fallback + missing-story recovery (`reader-journey`), viewport/responsive (`responsive-experience`), and the `VISIT_EVENT_LOG_LIMITS` history cap. Bundle is healthy (initial JS **634.96 / 700 KiB**; story/3D/Sentry all lazy). The real throttled LCP/CLS numbers are **environment-limited in the sandbox** and run on real hardware in CI. The app makes **zero network calls**, so it works offline once loaded (no offline _reload_ without a SW). New in-repo work: an **optional-asset-failure readability** test (block the 3D chunk / fonts → 2D stays readable), an **offline-after-load** test, a **large-history** load test, and a lightweight backgrounding check; a re-measurement record (owner/CI numbers); and the **no-SW re-confirmation**. Owner fork: re-confirm **no service worker for v1** (recommend; re-confirms 8.1). The budget pass/waiver is decided against real CI/device numbers.
