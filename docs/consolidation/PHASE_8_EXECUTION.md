...
- **Caching / redirects / 404** — `public/_headers` gains immutable `/assets/*` + `/fonts/*` and `no-cache` HTML; `public/_redirects` (`www.narramorph.com → narramorph.com` 301); a branded `public/404.html`. `.gitignore` updated so `_redirects`/`404.html` are tracked (Vite copies them to the deploy root). **Post-deploy correction (2026-07-18):** the first deploy attempt was on a **Workers** project, which rejected the cross-host `www → apex` line in `_redirects` (`Only relative URLs are allowed [code 100324]`) and then could not locate an assets directory (Workers expects a `wrangler` entry-point/assets binding). Resolved by (a) moving canonical-host redirection to a **zone-level Redirect Rule** (edge-native, host-agnostic), leaving `_redirects` comment-only, and (b) deploying as a **Cloudflare Pages** project (build `npm run build`, output `dist`) — the config-free path for this static-only app, matching the runbook. Runbook updated in `docs/RELEASE_ROLLBACK.md` §1.
