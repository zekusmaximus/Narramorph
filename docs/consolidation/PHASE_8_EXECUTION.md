...
**Design proposed on the feature branch; awaiting owner decisions.** Design: **[PHASE_8_2_SECURITY_PRIVACY.md](PHASE_8_2_SECURITY_PRIVACY.md)**. Grounded audit findings: security headers are greenfield; CSP is feasible at **`script-src 'self'`** (no inline script / no eval in the built app) with a documented **`style-src 'self' 'unsafe-inline'`** (runtime style injection by framer-motion/@xyflow/three + the FOUC inline block); rendering (React auto-escape, no `dangerouslySetInnerHTML`, no Markdown renderer) and the print-HTML export (`escapeHtml` on every value) are **already safe**, so sanitization focuses on the **save-file import** path (`JSON.parse` → `prepareSavedState` → `Object.assign`) — prototype pollution, oversized, malformed, HTML-in-prose. The one external origin is **Google Fonts** in `index.html` (a CSP + privacy decision). Host: **Cloudflare Pages**, **custom domain** (HSTS with preload eligibility). Owner forks: (1) **self-host Inter** vs. allowlist Google Fonts (recommend self-host); (2) **privacy-policy controller/contact** (owner-provided); (3) CSP enforce vs. report-only (recommend enforce); (4) security headers now vs. also caching headers (recommend security only; caching with 8.4). Implementation (headers file, self-hosted fonts, import sanitization + tests, privacy + header-checklist docs, an owner-run header verifier) proceeds after confirmation.
