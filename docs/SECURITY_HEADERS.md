...
```
default-src 'self'; script-src 'self' blob:; style-src 'self' 'unsafe-inline'; img-src 'self' data:;
...
- **`script-src 'self' blob:`** — contains **no** `'unsafe-inline'` / `'unsafe-eval'` and allows no remote script origin. The `blob:` source is required because Drei's text renderer uses `troika-worker-utils`: its worker bootstraps from a blob URL, then calls `importScripts` for blob-backed worker modules. Browsers apply `script-src` (not only `worker-src`) to those nested imports. Keep this exception paired with `worker-src blob:` and do not broaden it to `data:`, remote hosts, inline script, or eval. The built `index.html` itself still loads only Vite's same-origin hashed module.
...
- **`worker-src 'self' blob:`** — the opt-in 3D view spawns the outer text worker from a blob URL; `script-src blob:` separately permits that worker's nested blob module imports. The 2D reader (the default and the accessible path) uses no workers.
