...
function error(arr, msg) { arr.push(msg); }
...
function validateL2(meta, filename) {
  ...
  // Required identity
  if (!meta.variationId || typeof meta.variationId !== 'string') error(errors, 'variationId missing');
  if (!meta.nodeId || typeof meta.nodeId !== 'string') error(errors, 'nodeId missing');
  if (!meta.character || !CONFIG.characterSet.has(meta.character)) error(errors, 'character missing/invalid');
  if (meta.layer !== 2) error(errors, 'layer must be 2');
  if (!meta.pathPhilosophy || !CONFIG.pathSet.has(meta.pathPhilosophy)) error(errors, 'pathPhilosophy missing/invalid');
  if (!meta.transformationState || !CONFIG.stateSet.has(meta.transformationState)) error(errors, 'transformationState missing/invalid');
  ...
  if (!Array.isArray(meta.awarenessRange) || meta.awarenessRange.length !== 2) {
    ...
    if (typeof min !== 'number' || typeof max !== 'number') error(errors, 'awarenessRange values must be numbers');
    const band = meta.transformationState === 'firstRevisit' ? [21, 60] : [61, 100];
    if (!(min >= band[0] && max <= band[1])) warn(warnings, `awarenessRange ${min}-${max} outside expected band ${band[0]}-${band[1]}`);
  }

  if (typeof meta.wordCount !== 'number' || meta.wordCount <= 0) warn(warnings, 'wordCount missing or non-positive');
  if (!meta.createdDate || typeof meta.createdDate !== 'string') warn(warnings, 'createdDate missing');

  // Thematic + hints minimal set
  const primaryThemes = meta?.thematicContent?.primaryThemes;
  if (!Array.isArray(primaryThemes) || primaryThemes.length === 0) warn(warnings, 'thematicContent.primaryThemes empty');

  const keyPhrases = meta?.generationHints?.keyPhrases;
  if (!Array.isArray(keyPhrases) || keyPhrases.length === 0) warn(warnings, 'generationHints.keyPhrases empty');

  // Flags for batch placeholders
  const hasReviewRequired = JSON.stringify(meta).includes('REVIEW_REQUIRED');
  if (hasReviewRequired) warn(warnings, 'Contains REVIEW_REQUIRED placeholders');
...
function main() {
  ...
  let total = 0, ok = 0, invalid = 0, warned = 0, skipped = 0;
  const report = [];

  for (const file of files) {
    const base = path.basename(file);
    const isL2 = CONFIG.filenamePatternL2.test(base);
    if (!isL2) { skipped++; continue; }
    ...
    if (errors.length === 0 && (!strict || warnings.length === 0)) ok++;
    else if (errors.length === 0) { warned++; }
    else { invalid++; }
