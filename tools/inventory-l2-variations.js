...
function estimateAwarenessRange(transformationState, indicators) {
  if (transformationState === 'firstRevisit') {
    const level = indicators.filter(i => 
      i.startsWith('cross-character') || i === 'temporal-awareness'
    ).length;
    
    if (level === 0) return [21, 30];
    if (level === 1) return [31, 40];
    if (level === 2) return [41, 50];
    return [51, 60];
  } else {
    const level = indicators.filter(i => 
      i === 'frame-consciousness' || i === 'high-integration'
    ).length;
    
    if (level === 0) return [61, 70];
    if (level === 1) return [71, 80];
    if (level === 2) return [81, 90];
    return [91, 100];
  }
}
  ...
  for (const searchPath of CONFIG.searchPaths) {
    ...
    for (const filename of files) {
      ...
      if (!inventory.byState[parsed.transformationState]) {
        inventory.byState[parsed.transformationState] = { total: 0, withMetadata: 0, withoutMetadata: 0 };
      }
...
function printReport(inventory) {
  ...
  console.log(`  ⚠️  Without metadata: ${inventory.withoutMetadata} (${((inventory.withoutMetadata/inventory.totalFiles)*100).toFixed(1)}%)`);
  console.log('');
  ...
  // Sample analysis
  const samplesWithoutMetadata = inventory.files
